...
V5 adds LocationContext persistence and nullable `root_path_dialect`, `bound_location_context_id`, and `binding_evidence_json` storage on Source. Explicit ACTIVE creation, acceptance, retirement, and same-anchor replacement use the LocationContext table; first-time binding now uses the Source binding columns. Newly registered Sources remain unbound, and `root_path_dialect = NULL` means the legacy root/key has not been established under the future resolver contract. The current registration, discovery, reconciliation, hashing, and metadata paths do not query LocationContext or interpret these fields. No context is created automatically.
...
The current operational model remains `Source -> FileEntry -> ContentRecord`; its Source-owned FileEntries, FileEntry presence, and version-2 SCAN lifecycle are implemented behavior. V5 provides LocationContext and nullable Source binding persistence; explicit context creation, acceptance, retirement, same-anchor replacement, and first-time Source binding are implemented. The following wider target must not be read as implemented binding orchestration, unbinding/rebinding, SourceMembership, FileEntry-identity, presence-authority, or scan behavior.
...
Target FileEntry identity is source-independent: a surrogate ID plus `location_identity_status`, `location_context_id`, lossless `location_path`, and versioned equality `location_key`. Resolved identities will be unique by `(location_context_id, location_key)`; ambiguous legacy cases remain `UNRESOLVED` rather than being guessed or merged. The implemented pure layer defines `location_path` and `location_key`, but no current table field is populated through it. It supports only explicitly accepted address syntax, does not globally lowercase or Unicode-normalize, and compares ancestry by parsed dialect/root/component structure. Future resolution still must not use `toRealPath()`, treat hashes/inodes/file keys as durable identity, equate mapped drives with UNC paths automatically, or collapse distinct hard-link names. Rename/move detection and automatic remount recognition remain deferred.
...
The implemented pure `scan.authority` layer is a prerequisite, not an indexing caller. `ScanObservationAuthority.capture` checks supplied current Source/context rows with their persisted acceptance/binding authority and compares separate fresh typed APFS observations to those baselines. A valid legacy raw APFS acceptance document remains historical evidence but returns `STALE / CONTEXT_NOT_CURRENT`; malformed or contradictory current evidence retains integrity-failure semantics. `resolve` requires a regular non-link observation, exact structured absolute path/key, strict descent from the bound root, context containment, matching Source/context revisions, and host-supplied child-boundary classification on the accepted local APFS Volume UUID. `SourceRelativePath` subtracts exact structured components; its portable path/key follows today's exact-spelling rule. Thus overlapping Sources can produce the same absolute context/location candidate with different relative paths. The pure layer neither proves that a host classified a boundary correctly nor reads the filesystem; a later host adapter must classify every traversal branch. For this first profile any child mount is unsupported, including one reporting the same Volume UUID. An unsupported provider, changed volume, uncertain boundary, or link ambiguity fails closed. Unbound and unsupported-profile Sources cannot produce resolved candidates. Existing legacy FileEntries remain unresolved without a new trusted observation; the later admission versus unresolved-compatibility choice is open.
...
`ScanObservationAuthority.assessTraversal` is a separate pure missing-claim gate. It compares trusted start/end Source, context, Source-root baseline and fresh observations, plus the exact claimed scope. Only a complete traversal with no cancellation, error, inaccessible subtree, unsupported or uncertain child storage, or symbolic-link ambiguity receives missing-claim authority. Previously qualified positive observations remain representable if a later traversal cannot claim completeness. This gate is not connected to current reconciliation; future publication must also revalidate database authority in a short transaction.
