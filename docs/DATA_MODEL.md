...
V5 implements the LocationContext fields described above, and services enforce structural-domain overlap during ACTIVE creation and explicit guarded retirement. The future context baseline will protect a shared address domain, while a separate Source binding baseline will protect each recursive Source root. The pure path/key contract and the limited macOS exact-spelling/local-APFS probe are implemented as described above; atomic replacement, binding, and all other host/provider profiles remain open.
