...
## Staged LocationContext and Membership Migration
...
- Keep current registration and SCAN behavior unchanged after V5. FileEntry remains Source-owned, presence remains on FileEntry, indexing does not consult LocationContext, and no binding requirement becomes operational.
...
- Pair SourceMembership trusted-authority provenance in `observed_source_location_revision` and `observed_location_context_revision`: both are NULL for migrated or otherwise unproven historical memberships, or both are non-NULL for observations made under established Source and LocationContext authority. V6 must enforce this with a database CHECK constraint. Do not impose a paired-null constraint on `last_positive_scan_run_source_id` and `last_positive_traversal_generation`; V5 did not enforce that relationship, so V6 preserves those historical values conservatively.

V5 implements the LocationContext/Source binding storage described above; explicit ACTIVE context creation, acceptance, retirement, and same-anchor replacement now use the context table, while explicit first-time local APFS Source binding uses the V5 columns. The pure scan authority contract now classifies pre-captured local APFS observations and complete traversal evidence without persistence wiring. Indexing remains unchanged. Host-side child-boundary detection, future admission of unsupported/unbound Sources, binding continuity integration, legacy conflicts, historical Source revisions, and Windows junction/reparse-point handling remain cutover design work.
