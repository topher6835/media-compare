...
For this macOS/APFS profile, a persisted LocationContext anchor is a logical boundary in the user-visible namespace and need not equal the physical APFS mount path. `MacOsApfsLogicalAnchorResolver` discovers that boundary for an existing Source root by resolving its exact visible spelling, inspecting APFS identity at the root and each visible parent, and returning the highest ancestor with the same Volume UUID. A same-UUID parent with contradictory physical mount information is uncertain; a different UUID establishes the boundary even when mount text matches. All inspected paths and the root spelling are checked again before accepting the anchor. Physical mount information remains probe evidence, not a replacement Source path. Explicit Source preparation now uses this resolver; the resolver itself performs no persistence. Automatic remount recognition remains deferred.

`SourcePreparationService` now uses that logical resolver for explicit first-time preparation. Registration remains a database-only operation and can store an unavailable absolute path. Preparation selects an applicable ACTIVE context by structured visible anchor containment, reuses it when its current APFS authority is valid, or creates a REVIEW_REQUIRED context through guarded activation. It captures context evidence outside a write transaction, accepts a review-required context through the existing guarded service, captures fresh context and Source-root evidence, and invokes guarded first binding. Context selection never treats path text as physical continuity proof: the fresh APFS context and root probes must agree with current accepted authority. The response derives PREPARATION_REQUIRED, READY, or REBIND_REQUIRED from the durable Source shape; structured-unbound Sources cannot enter first binding. The UI gates Analyze on READY, while v3 backend admission remains authoritative. Rebinding UI and automatic remount recognition remain deferred.
...
The React frontend consumes these endpoints through small typed API modules. `/sources` lists and registers Sources using the backend's public fields and explicit absolute-path contract, then offers explicit first-time preparation. Analyze appears for READY Sources and starts a durable version-3 indexing run through `/api/indexing-runs`; the backend owns stage sequencing and the page reconstructs state through the source-status and detail polling APIs. Persisted stage labels, progress, typed summaries, and safe failures remain authoritative. Registration, preparation, and indexing perform no filesystem mutation.
...
`SourceBindingService.bindUnboundSource(...)` makes the first existing legacy Source to existing current ACTIVE + ACCEPTED context link for local macOS/APFS. Typed context and Source-root probe results are captured before its short transaction. Under the same SQLite writer reservation used by context transitions, it rereads both rows, validates the context through `LocationContextAcceptanceAuthority`, compares the fresh context observation with the accepted baseline, and validates one accepted Source-root candidate for exact configured/observed spelling, structural containment, matching APFS volume and classification, and provenance for the post-bind Source revision. That accepted candidate becomes the initial Source-root baseline; later continuity checks may compare it with later observations. A guarded Source update preserves `root_path` while setting Unix dialect, canonical `lk1` key, context foreign key, versioned binding envelope, revision N+1, and caller-owned non-regressing update time. A bound Source blocks context retirement/replacement. First binding is non-idempotent; general probe orchestration remains future work. V6 subsequently changed FileEntry ownership, membership presence, and new SCAN execution without changing first-binding semantics.

V7 records each successful first binding as one open `source_binding_period` in the same writer-reserved transaction as that Source update. It snapshots the resulting Source revision, context, root dialect/path/key, exact binding evidence, and bind timestamp. Migration backfills the currently open period from each bound Source row without probing or changing existing catalog rows. The Source row and `SourceBindingAuthority` remain the sole current binding authority for v3 admission and publication; the period table is durable lifecycle history. Explicit unbinding closes the period while withdrawing Source authority; explicit rebinding creates a new open period.

`SourceUnbindingService.unbind(...)` explicitly withdraws current authority without a filesystem probe. In one writer-reserved transaction it rereads the Source, requires its open period to match the bound snapshot, closes that period at Source revision N+1, retires every ACTIVE SourceMembership without changing presence or observation history, and clears only the Source's context and binding evidence. It preserves the structured root path, dialect, and canonical key while advancing the Source revision and timestamp once. A missing or contradictory period fails as an integrity error; write failure rolls the transition back. The closed period never grants current authority.

`SourceRebindingService.rebind(...)` accepts only a structured-unbound Source with no open period, matching latest closed history, and no ACTIVE memberships. It can target the same current accepted LocationContext or a different one after replacement. Fresh accepted context and Source-root observations are supplied before the transaction. Under the SQLite writer reservation, `SourceBindingValidation` applies the same APFS evidence checks used for first binding. Rebinding also requires the observed root to retain the configured spelling and existing canonical key. A guarded Source update advances revision once and writes new current context/evidence while preserving Source ID, root path, key, and dialect; insertion of a new open period commits with that update. Retired memberships and historical FileEntry/content/analysis stay unchanged. A later trusted positive observation can reactivate the same membership for the same resolved FileEntry; a different context's FileEntry is not inferred equivalent from path or content. Closed periods alone never authorize v3 work.

`SourceRelocationService.relocateAndBind(...)` is an explicit caller-selected transition from structured-unbound at the old root to bound at a new root. It verifies that the latest closed period still matches the old Source root, revision, and timestamp, with no open period or ACTIVE memberships. It requires a selected current accepted context and fresh accepted context/root capture for the requested new root; the shared APFS validator uses that explicit root while first binding and fixed-root rebinding pass the persisted root. In one writer-reserved transaction, a dedicated guarded update changes Source root path/key, context/evidence, revision, and timestamp; it verifies the resulting bound Source and inserts one open period for the new root. Previous periods, retired memberships, FileEntries, content, and analyses remain unchanged. Old-root/context scan work cannot gain current authority from history. A later trusted scan may establish a new membership and FileEntry relationship without inferring equivalence to old-context history. No intermediate unbound-at-new-root state or automatic remount recognition is introduced.
...
The API can register a Source, list Sources in database-ID order, retrieve one Source by ID, and explicitly prepare a first-time Source for local macOS/APFS analysis. Registration does not check availability; preparation does. Source update, deletion, relocation, and remount recognition have no public API in this slice.
