...
V5 implements the LocationContext/Source binding storage described above; explicit ACTIVE context creation, acceptance, retirement, and same-anchor replacement now use the context table, while Source binding remains dormant. The remaining bullets are target product/runtime behavior. The pure key/path contract is used by the context lifecycle services; local macOS/APFS context evidence is now used by explicit acceptance, while Source-root comparison remains unwired to binding and indexing. Before any SourceMembership authority migration, define the remaining limited platform resolver/binding acceptance contract: filesystem spelling resolution and probing, binding continuity/context transitions, uncertain mountpoints, legacy conflicts, historical Source revisions, and relevant Windows junction/reparse-point behavior.
