...
The implemented `location` package is a pure, host-independent foundation for that future resolver. `LocationPathParser` accepts an explicit `unix`, `win-drive`, or restricted `win-unc` dialect; `LocationPath` retains exact root/component spelling and performs structural containment; and strict `lp1`/`lk1` codecs provide versioned portable representations. This layer uses no Java NIO `Path` or filesystem inspection and is not called by Source registration, persistence, or indexing. A later host resolver will verify exact enumerated spelling, existence, type, and link boundaries. A later provider continuity probe will separately collect platform evidence such as the tested macOS/APFS profile. Neither later boundary is implemented yet.

The package also implements the pure evidence boundary for the tested local macOS/APFS profile. Separate typed context-anchor and Source-root records retain authoritative structured paths/keys, canonical UUIDs, inode and Source birth-time evidence, classification flags, revisions where applicable, acceptance timestamps, and bounded optional context diagnostics. Strict deterministic codecs reject unknown or malformed schema and contradictory path/key pairs. Pure comparison ignores timestamps and diagnostics, returns only bounded outcome/reason enums, checks Source-root containment structurally, and performs no mutation. These records and comparators are not persistence validation or live trust: exact-spelling resolution, filesystem/process evidence collection, pre/post probing, transactional revision revalidation, and operational transitions remain unimplemented.
