...
- Introduce a durable `LocationContext` entity for one accepted continuity period of one address/binding domain. The previous catalog-wide embedded active-context proposal is superseded because unrelated storage domains need independent trust lifecycles. A New Catalog starts with zero contexts. Initial domains are one per Windows drive root, one per supported UNC server/share, and an explicitly selected Unix/macOS storage anchor; active domains must not overlap.
