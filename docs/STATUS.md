...
Unbinding validation: `mvn -q -Dtest=SourceUnbindingServiceTests,SourceBindingServiceTests,SourceBindingPeriodMigrationTests,SourceMembershipAuthorityTests,LocationContextRetirementServiceTests,LocationContextReplacementServiceTests,Version3ExecutionTests test` passed 96 tests with no failures, errors, or skips. After adding a restrictive history-FK assertion, `mvn -q -Dtest=SourceUnbindingServiceTests test` passed its 11 tests. The full backend and frontend gates were not rerun for this slice.
...
- Only the tested local macOS/APFS binding profile can execute v3 scans. Windows, network, and other provider profiles remain future work. Rebinding, automatic membership reactivation, multiple active catalogs, catalog switching, automatic historical merge, and the shortened SCAN are not implemented.
...
Design and implement Source rebinding as a separate lifecycle slice, using preserved structured root and closed binding history while keeping the Source row authoritative.
