...
V5 implements the LocationContext fields described above but not their production semantics. The future context baseline will protect a shared address domain, while a separate Source binding baseline will protect each recursive Source root. The pure path/key contract and the limited macOS exact-spelling/local-APFS probe are implemented as described above; transition rules, structural-domain overlap enforcement, binding, and all other host/provider profiles remain open.
