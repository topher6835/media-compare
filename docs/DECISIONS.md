...
- Establish exact-spelling resolution, production continuity probing, structural-overlap transitions, and explicit binding contracts before changing indexing authority.
...
V5 implements only the dormant LocationContext/Source binding storage described above. The remaining bullets are target product/runtime behavior. The pure key/path and local macOS/APFS evidence/comparison contracts are implemented but unwired. Before any SourceMembership authority migration, define the remaining limited platform resolver/binding acceptance contract: filesystem spelling resolution and probing, binding continuity/context transitions, uncertain mountpoints, legacy conflicts, historical Source revisions, and relevant Windows junction/reparse-point behavior.
